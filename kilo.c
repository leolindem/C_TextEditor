...
char *editorRowsToString(int *buflen){
    ...
    // Add the length of each row + newline char
    for (j = 0; j < E.numrows; j++){
        totlen += E.row[j].size + 1;
    }
    *buflen = totlen;

    // Allocates a buffer for all the lines
    char *buf = malloc(totlen);
    char *p = buf;

    // Loop through all rows and memcpy them to the end of the buf
    for (j = 0; j < E.numrows; j++){
...
void editorOpen(char *filename)
{
    ...
    // pointer to file that we are reading in
...
{
    static int quit_times = KILE_QUIT_TIMES;
    ...
    case CTRL_KEY('q'):
        // If the file is dirty we have to CRLT Q 3 times
